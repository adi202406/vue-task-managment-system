...
function resetForm() {
  ...
  form.color = props.label?.color || '#3b82f6'
...
async function handleSave() {
  ...
  try {
    ...
    formError.value = error instanceof Error ? error.message : 'Failed to save label.'
  } finally {
      ...
      <div v-if="show" class="fixed inset-0 z-50 grid place-items-center bg-black/60 px-4 py-4 backdrop-blur-sm" @click.self="handleClose">
        <form class="flex w-full max-w-md flex-col rounded-2xl border border-white/10 bg-[rgba(5,10,24,0.96)] shadow-[0_24px_90px_rgba(0,0,0,0.5)] backdrop-blur-md" @submit.prevent="handleSave">
          <div class="flex items-center justify-between border-b border-white/10 p-5">
            <div>
              <p class="text-[10px] uppercase tracking-[0.25em] text-blue-400">{{ label ? 'Update' : 'New' }} Label</p>
              <h2 class="mt-1 text-lg font-bold text-white">{{ label ? 'Edit Label' : 'Create Label' }}</h2>
            </div>
          ...
          <div class="p-5 space-y-4">
            ...
            <div>
              <label class="block text-sm font-semibold text-slate-300 mb-1.5" for="label-name-input">Label Name</label>
              <input
                v-model="form.name"
                required
                type="text"
            ...
            <div>
              <label class="block text-sm font-semibold text-slate-300 mb-1.5" for="label-desc-input">Description (optional)</label>
              <textarea
                v-model="form.description"
                id="label-desc-input"
                rows="3"
                class="w-full resize-none rounded-lg border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm text-white placeholder-slate-600 outline-none focus:border-blue-500 transition"
