...
// Saat halaman dimuat, cek apakah ada token OAuth yang embedded di URL
onMounted(async () => {
  ...
  if (embeddedPayload) {
    ...
    try {
      ...
      errorMessage.value = error instanceof Error ? error.message : 'Login Google gagal diproses.'
...
<template>
  <!-- Loading state saat memproses OAuth callback -->
  <main v-if="isProcessingOAuth" class="flex min-h-screen items-center justify-center bg-[#030712] px-6 text-white">
    <div class="w-full max-w-lg rounded-[28px] border border-white/10 bg-white/[0.04] px-8 py-10 text-center shadow-[0_30px_80px_rgba(0,0,0,0.28)]">
      <div v-if="errorMessage" class="space-y-4">
        <h1 class="text-2xl font-semibold">Login Google gagal</h1>
      ...
      <div v-else class="space-y-4">
        ...
        <h1 class="text-2xl font-semibold">Memproses login Google</h1>
  ...
  <AuthLayout
    ...
    active-tab="login"
    title="Welcome back"
    subtitle="Sign in to continue to Nexora."
  >
    <div class="mt-10 space-y-7">
      <div v-if="successMessage" class="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-5 py-4 text-base text-emerald-200">
        {{ successMessage }}
      </div>
      ...
      <form class="space-y-6" @submit.prevent="handleSubmit">
        ...
        <div class="flex items-center justify-between gap-4 text-lg">
          <label class="flex items-center gap-3 text-slate-300">
            <input v-model="form.remember" type="checkbox" class="h-5 w-5 rounded border-white/20 bg-white/10 accent-[#2563eb]">
            Remember me
          </label>
          <span class="text-[#2f6bff]">Forgot password?</span>
        </div>
      ...
      <p class="pt-4 text-center text-xl text-slate-400">
        Don't have an account?
        <RouterLink to="/register" class="ml-2 text-[#2f6bff]">Create account -&gt;</RouterLink>
      </p>
    </div>
  </AuthLayout>
</template>
